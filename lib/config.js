...
if (fs.existsSync(path.join(os.homedir(), ".mpx", "credentials"))) {
  fs.moveSync(mpxConfigFile, mpxConfigFile);
}
...
async function update(env) {
  try {
    if (!fs.existsSync(mpxConfigFile)) {
      fs.outputFileSync(mpxConfigFile, "");
    }
    const existing = dotenv.parse(await fs.readFile(mpxConfigFile, "utf-8"));
  ...
  const contents = Object.keys(env)
    ...
    .join("\n");
  await fs.writeFile(mpxConfigFile, contents);
...
async function checkEnvironmentVariables() {
  if (!("MATHPIX_OCR_API_KEY" in process.env) && !("MATHPIX_SNIP_AUTH_TOKEN" in process.env)) {
    console.log(`You must connect mpx cli to your Mathpix Snip or OCR API.`);
    console.log(
      `To connect to your Mathpix Snip account sign up at https://accounts.mathpix.com then run mpx login.`
    );
    console.log(
      `To connect to your Mathpix OCR API create an API key at https://accounts.mathpix.com/ocr-api then`
    );
    console.log(
      `set the API KEY to the environment variable "MATHPIX_OCR_API_KEY" or run mpx set-api-key YOUR_API_KEY`
    );
    ...
    const answers = await inquirer.prompt([
      {
        type: "confirm",
        name: "pricing",
        message: "Do you agree to use Mathpix Snip account for mpx cli usage?",
      },
    ]);
    if (!answers.pricing) {
      console.log(`You must agree to use your Mathpix Snip Account for mpx usage.`);
      return process.exit(1);
    }
...
async function loginToMathpixSnipAccount() {
  ...
  console.log(`Saved Mathpix Snip authorization token to: ${mpxConfigFile}`);
  return ;
}
...
async function logoutOfMathpixSnipAccount() {
      ...
      if (jsonResponseBody["errors"] && jsonResponseBody["errors"].length) {
        for (let i = 0; i < jsonResponseBody["errors"].length; i++) {
          ...
          if (error["id"] === "invalid_access_token") {
            ...
            console.log(`Removed Mathpix Snip authorization token from: ${mpxConfigFile}`);
          } else {
            console.log(
              `Unexpected error, please submit a bug report at: ${pkg.repository.issues}`
            );
  ...
  console.log(`Logged out and removed Mathpix Snip authorization token from: ${mpxConfigFile}`);
}
