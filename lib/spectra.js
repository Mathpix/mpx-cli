...
const credentialsFile = path.join(os.homedir(), ".spectra", "credentials");
dotenv.config({ path: credentialsFile })
...
try {
    ...
    console.log(`There was an unexpected error, you can try again or if it continues please submit`);
...
program
  ...
  .action(async (key, options) => {
    ...
    try {
      fs.outputFileSync(credentialsFile, `MATHPIX_OCR_API_KEY=${key}`)
    } catch (err) {
      debugLog(err)
      console.log(`Could not write to credentials file: ${credentialsFile}`);
      return process.exit(1);
    }

    console.log(`Saved api key in credentials file: ${credentialsFile}`);
