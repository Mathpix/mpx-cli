...
fs.writeFileSync(__dirname + '/../assets/css/mathpix_styles.css', mathpixStyles);
...
program
    ...
    .action((opts) => {
        ...
        fs.writeFileSync(path.join(destDir, 'navigation.json'), JSON.stringify(treeDirectory));
        ...
        if (!DEV_MODE) {
            fs.copySync(path.join(__dirname, '..', 'assets', 'css'), path.join(outputDirectory, 'assets/css'));
            fs.copySync(path.join(__dirname, '..', 'assets', 'scripts'), path.join(outputDirectory, 'assets/scripts'));
