  ...
  .action((opts) => {
    ...
    if (!fs.existsSync(configFile) || DEV_MODE) {
      if(DEV_MODE) {
        ...
        console.log(
          chalk.greenBright.bold(`DEV mode: Removed .spectra directory at ${destDir}.`)
        );
      }
      console.log(
    ...
    if (!fs.existsSync(indexFile) && !fs.existsSync(indexFileMMD)) {
      fs.copySync(
        path.join(__dirname, "..", "templates", "index-template.md"),
        path.join(inputDirectory, "index.md")
      );
      console.log(`Created new file at ${indexFile}.`);
    }
    ...
    const treeDirectory = createTree(inputDirectory, outputDirectory);
    fs.writeFileSync(
      path.join(destDir, 'data', "navigation.json"),
