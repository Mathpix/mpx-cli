...
pleaseUpgrade(pkg, {
  message: function (requiredVersion) {
    return `mpx requires Node v ${requiredVersion}. Please upgrade to use it.`;
  },
});
...
const mpxConfigFile = path.join(os.homedir(), ".mpx", "config");
dotenv.config({ path: mpxConfigFile });
...
program
  ...
  .action((source, destination, opts) => {
    ...
    const srcDir = path.join(__dirname, "..", ".mpx");
    const destDir = path.join(inputDirectory, ".mpx");
    const configFile = path.join(inputDirectory, ".mpx", "config.js");
...
program
  ...
  .action(async (source, destination, options) => {
    try {
      ...
      const validSourceExtensions = [".mmd", ".md", ".pdf", ".jpg", ".jpeg", ".png"];
      ...
      switch (true) {
        ...
        case sourceExt === ".pdf" && destinationExt === ".tex":
          ...
          break;
        case (sourceExt === ".png" || sourceExt.startsWith(".jp")) &&
          (destinationExt === ".mmd" || destinationExt === ".md"):
          await convert.ImageToMMD(source, destination);
          break;
        case (sourceExt === ".png" || sourceExt.startsWith(".jp")) && destinationExt === ".html":
          await convert.ImageToHTML(source, destination);
          break;
        case (sourceExt === ".png" || sourceExt.startsWith(".jp")) && destinationExt === ".docx":
          await convert.ImageToDOCX(source, destination);
          break;
        case (sourceExt === ".png" || sourceExt.startsWith(".jp")) && destinationExt === ".tex":
          await convert.ImageToTEX(source, destination);
          break;
        case (sourceExt === ".png" || sourceExt.startsWith(".jp")) && destinationExt === ".pdf":
          if (options.pdfMethod === "html") {
            await convert.ImageToHTMLPDF(source, destination);
            break;
          } else {
            await convert.ImageToLatexPDF(source, destination);
            break;
          }
          break;
...
program
  ...
  .description("save Mathpix OCR API key")
  .option("-v, --verbose", "enable verbose mode to add more logging")
  .action(async (key, options) => {
    ...
    try {
      ...
      debugLog(err);
      console.log(`Could not write to: ${mpxConfigFile}`);
      return process.exit(1);
    }

    console.log(`Saved api key to: ${mpxConfigFile}`);
  });

program
  .command("unset-api-key")
  .description("remove Mathpix OCR API key")
  .option("-v, --verbose", "enable verbose mode to add more logging")
  .action(async (key, options) => {
    let debugLog = function () {};
    ...
    try {
      await config.remove("MATHPIX_OCR_API_KEY");
    } catch (err) {
      debugLog(err);
      console.log(`Could not write to: ${mpxConfigFile}`);
      return process.exit(1);
    ...
    console.log(`Removed api key from: ${mpxConfigFile}`);
  });

program
  .command("login")
  .description("log into your Mathpix Snip account and save authorization token")
  .option("-v, --verbose", "enable verbose mode to add more logging")
  .action(async (options) => {
    ...
    await config.loginToMathpixSnipAccount();
  });

program
  .command("logout")
  .description("log out of your Mathpix Snip account and remove authorization token")
  .option("-v, --verbose", "enable verbose mode to add more logging")
  .action(async (options) => {
    let debugLog = function () {};
    ...
    await config.logoutOfMathpixSnipAccount();
  });
